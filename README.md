...
**Windows:** дважды кликните `run.bat`. При первом запуске он сам создаст venv,
установит зависимости, сделает `.env` из примера, запустит сервер и откроет браузер.
...
| `MAX_SECTION_PAIRS` | Потолок пар секций в режиме sectioned (защита от DoS) |

## Безопасность

- Ответ модели рендерится как Markdown и **санитизируется через DOMPurify** —
  `<script>`/`<img onerror>` из ответа или содержимого документа не выполнятся.
  Имена файлов экранируются перед вставкой в DOM.
- Загружаемые файлы ограничены по размеру (`MAX_FILE_MB`, проверка до чтения в
  память) и должны иметь расширение `.docx`. Число секций в режиме sectioned
  ограничено `MAX_SECTION_PAIRS`.
- `.docx` — это zip-архив; не открывайте заведомо вредоносные документы.
- Эндпоинты не защищены аутентификацией — приложение рассчитано на **локальный
  запуск** (`127.0.0.1`). Не выставляйте его в открытую сеть без обратного прокси
  с авторизацией: `/api/health` раскрывает имя модели и base_url.
